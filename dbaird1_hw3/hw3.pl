...
%:- is_increasing([1,2,3,1]).    % FAIL
%:- is_increasing([3,2,1]).      % FAIL
%:- is_increasing([7,1]).        % FAIL
...
element_at(X, [Y|Ys], 0):-
    Y==X.
element_at(X, [Y|Ys], N):-
    N\=0, write(Y), nl, Nx is N-1,nl, element_at(X, Ys, Nx).
...
insert_at(X,[YY|Y],K,[YY|Z]):- K > 1, !,
    Kn is K - 1, insert_at(X,Y,Kn,Z).
...
replicated(X,Y):-
  =(R, []), connector(X, R), compare(R, Y).

connector([X|Xs], R):-
    =(Q, []), =(P, []), builder(X, X, P),  append(R, P, Q), append([],Q,R), connector(Xs, M).

builder(X, C, P):-
  C==0.
builder(X, C, P):-
  =(W, [X]), append(P, W ,F), =(P, []),=(K, []), append(K,F,X),write(X), V is C-1, C > 0, builder(X, C-1, P).
...
reverse_list(F, R) :-
rev_help(F,N,[]), compare(N, R).

rev_help([],N,N) :- !.
rev_help([X|Xs],N,A) :- rev_help(Xs,N,[X|A]).
